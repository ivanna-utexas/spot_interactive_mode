...
REQUIREMENTS_FILE=/tmp/spot_wrapper_requirements.txt
...
# the following code  takes a lot of time to run, so it's commented out for now and replaced the one with below
...
source /opt/ros/humble/setup.bash && rosdep update

WS=${WS:-/home/ros/masa_ws}
if compgen -G "$WS/src/*/package.xml" >/dev/null; then
  rosdep install --from-paths "$WS/src" --ignore-src -y -r --rosdistro=humble
fi
# replaced until here
